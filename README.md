...
- **v1.0.10**：性能修复——打开会话不再卡几秒（响应体读取+JSON 解析整体移出主线程）；流式输出改为 50ms 批量更新（不再每 token 全列表重组）且长回复只渲染尾部，避免越流越卡；自动滚动仅在底部时跟随，不再抢用户上翻；打开运行中会话不再丢失流式内容（加载期间事件缓冲重放 + 历史尾页补 chunk 分支）
...
- 最新构建产物：[release/DSH-Remote-v1.0.10.apk](release/DSH-Remote-v1.0.10.apk)（debug 签名，Android 10+）

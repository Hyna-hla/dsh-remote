...
window.__ModuleLoader__.load({
    ...
    factory: function (require) {
        ...
        function removePairDialog() {
            if (pairDialogEl && pairDialogEl.parentNode)
                pairDialogEl.parentNode.removeChild(pairDialogEl);
            pairDialogEl = null;
        ...
        function PairSection() {
            ...
            return h("div", { style: S.root }, h("div", { style: S.title }, "配对管理（手机 App）"), h("div", { style: S.sub }, "手机 App 首次连接本电脑时，需在此确认允许；已允许的设备可随时撤销，撤销后下次连接需重新确认。"), h("div", { style: S.card }, devices === null ? h("div", { style: S.sub }, "加载中…") :
                devices.length === 0 ? h("div", { style: S.sub }, "暂无已配对设备。") :
                    devices.map(function (d) {
                        return h("div", { key: d.deviceId, style: Object.assign({}, S.row, { justifyContent: "space-between", marginBottom: 8 }) }, h("div", { style: S.label }, d.name || d.deviceId), h("button", { style: S.btn, disabled: busyId === d.deviceId, onClick: function () { revoke(d.deviceId); } }, busyId === d.deviceId ? "撤销中…" : "撤销"));
                    })));
        }
        ...
        function PairCodeCard() {
                    ...
                    setLeft(function (prev) {
                        if (prev <= 1) {
                            ...
                            fetchJson("/api/remote-access/pair/code/current")
                                .then(function (nr) { if (nr && nr.ok && nr.code)
                                arm(nr); })
            ...
            function generate() {
                ...
                fetchJson("/api/remote-access/pair/code/generate", {
                    ...
                    .then(function (r) { if (r && r.ok && r.code)
                    arm(r); })
                    .catch(function () { })
            ...
            useEffect(function () {
                fetchJson("/api/remote-access/pair/code/current")
                    ...
                    arm(r); })
                    .catch(function () { });
                return function () { if (timer.current)
                    clearInterval(timer.current); };
            }, []);
            var mm = String(Math.floor(left / 60)).padStart(2, "0");
            var ss = String(left % 60).padStart(2, "0");
            return h("div", { style: S.card }, h("div", { style: S.row }, h("div", { style: S.title }, "配对码（推荐）"), pc ? h("span", { style: Object.assign({}, S.badge, { background: "rgba(52,199,89,.15)", color: "var(--dsw-alias-state-success-primary)" }) }, "有效 " + mm + ":" + ss) : null), h("div", { style: S.sub }, "生成一个 6 位随机配对码，在手机 App 连接本机后输入即完成配对——不用再守着确认框点允许。"), pc ? h("div", { style: Object.assign({}, S.mono, { fontSize: 30, fontWeight: 700, letterSpacing: "0.4em", color: "var(--dsw-alias-brand-primary)", marginTop: 8, marginBottom: 2 }) }, pc.code) : null, pc ? h("div", { style: S.sub }, "10 分钟有效 · 最多试错 5 次 · 验证通过立即作废") : null, h("div", { style: Object.assign({}, S.row, { marginTop: 10 }) }, h("button", { style: S.btnPrimary, disabled: busy, onClick: generate }, busy ? "生成中…" : pc ? "重新生成" : "生成配对码")));
        }
        ...
        function TrustedHostsCard() {
            ...
            return h("div", { style: S.card }, h("div", { style: S.row }, h("div", { style: S.title }, "公网域名白名单"), hosts !== null ? h("span", { style: Object.assign({}, S.badge, { background: "rgba(142,142,147,.15)", color: "var(--dsw-alias-label-secondary)" }) }, "共 " + hosts.length + " 条") : null), h("div", { style: S.sub }, "手机通过公网隧道连接时，DSH 核心校验请求的 Host 域名（防 DNS 重绑定）；把隧道的公网域名加进来手机才能连上。填「域名」或「域名:端口」（不带 https://；中文域名请用 punycode）。"), hosts === null ? h("div", { style: S.sub }, "加载中…") :
                hosts.length === 0 ? h("div", { style: S.sub }, "暂无白名单条目（手机只能走局域网，或重写 Host 为 localhost 的隧道）。") :
                    hosts.map(function (x) {
                        return h("div", { key: x, style: Object.assign({}, S.row, { justifyContent: "space-between", marginBottom: 6 }) }, h("span", { style: S.mono }, x), h("button", { style: S.btn, disabled: busy, onClick: function () { mutate("remove", x); } }, "删除"));
                    }), h("div", { style: Object.assign({}, S.row, { marginTop: 10 }) }, h("input", { style: Object.assign({}, S.input, { width: 230 }), placeholder: "如 tunnel.example.com 或 域名:8080", value: input, onChange: function (e) { setInput(e.target.value); } }), h("button", { style: S.btnPrimary, disabled: busy || !input.trim(), onClick: function () { mutate("add", input.trim()); } }, busy ? "处理中…" : "添加")), err ? h("div", { style: Object.assign({}, S.err, { marginTop: 6 }) }, err) : null, h("div", { style: S.sub, marginTop: 6 }, "✔ 白名单写入 connection 行的 trustedHosts（拼回 LAN 地址与 --trusted-host），修改后立即生效（DSH 热监视 cordis.patch.yml）；如未生效请重启 DSH。白名单只解决 Host 校验，手机请求仍需配对拿到通道 token。"));
        }
        // ---------- 设置页「远程控制」：远程互信认证 ----------
        function TrustSection() {
            return h("div", { style: S.root }, h("div", { style: S.row }, h("div", { style: S.title }, "远程互信认证"), h("span", { style: Object.assign({}, S.badge, { background: "rgba(52,199,89,.15)", color: "var(--dsw-alias-state-success-primary)" }) }, "● 已启用")), h("div", { style: S.sub }, "插件只负责远程互信：手机 App 首次连接后经配对码（或 PC 确认框）完成配对，获得远程通道 token；此后所有 /api 请求与实时通道（WebSocket）都要求该 token，拿到地址的陌生人无法遥控这台电脑。"), h("div", { style: S.ok }, "✔ 通道鉴权已挂载：本机浏览器放行；手机引导端点（配对握手、配对码校验、host.describe）豁免；其余 /api（含配对管理操作）一律需要 Bearer token。"), h(PairCodeCard, {}), h(TrustedHostsCard, {}), h("hr", { style: S.divider }), h(PairSection, {}));
        }
        function apply(ctx) {
            ctx.slots.inject("settings.section", function () {
                return ctx.slots.register({ name: "settings.section", id: "dsh-remote-access", order: 40, label: function () { return "远程控制"; } }, TrustSection);
            });
        }
        exports.apply = apply;
